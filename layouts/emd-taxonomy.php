...
<?php 
	$has_sidebar = apply_filters( 'emd_show_temp_sidebar', 'right', 'wpas_attendeeinfo', 'taxonomy');
	if($has_sidebar ==  'left'){
		do_action( 'emd_sidebar', 'wpas_attendeeinfo' );
	}
	if($has_sidebar == 'full'){
?>
...
<?php
	...
	while ( have_posts() ) : the_post(); ?>
			...
			<?php emd_get_template_part('wpas_attendeeinfo', 'taxonomy', str_replace("_","-",$queried_object->taxonomy . '-' . $post->post_type)); ?>
			</div>
                <?php endwhile; // end of the loop. ?>
<?php	$has_navigation = apply_filters( 'emd_show_temp_navigation', true, 'wpas_attendeeinfo', 'taxonomy');
	if($has_navigation){	
		global $wp_query;
