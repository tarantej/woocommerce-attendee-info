...
<?php 
	$has_sidebar = apply_filters( 'emd_show_temp_sidebar', 'right', 'wpas_attendeeinfo', 'single');
	if($has_sidebar ==  'left'){
...
<?php
	...
	while ( have_posts() ) : the_post(); ?>
		...
		<?php emd_get_template_part('wpas_attendeeinfo', 'single', str_replace("_","-",$post->post_type)); ?>
		</div>
		<?php
		...
		?>
<?php 	$has_navigation = apply_filters( 'emd_show_temp_navigation', true, 'wpas_attendeeinfo', 'single');
	if($has_navigation){
		global $wp_query;
